...
comment columns of my rubric when I was half done grading an assignment and
then recopy these columns into the students partially graded answer sheets. I
regularly regraded many of the students I looked at initially after having
...
After a while I realized that my primary frustration in grading student papers
...
efficiently keep track of all the small details, notes, and odd comments. This
was my motivation for developing Rgen.
...
from the screencapture with a grey box. This birds eye perspective shows per
...
fixed point scores with subtraced point values, although these are supported.

While MyCanvas will not export grades on a per question basis it will export
summary data for all student evaluations. In the case of multiple choice quizzes which are
...
calculation for a student listing scores for the first few assignments and the
first test. The second term test, final assignment and final exam have not yet
